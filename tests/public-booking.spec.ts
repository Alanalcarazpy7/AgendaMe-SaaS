...
import { abrirDashboardComoAdmin, crearReservaPublica } from "./helpers/agendame";
...
test("cliente puede crear reserva pública y admin la ve en reservas", async ({ page, browser }) => {
  ...
  await expect(adminPage.locator("body")).toContainText("Barberia1");
  await expect(adminPage.locator("body")).toContainText("Secundaria");
